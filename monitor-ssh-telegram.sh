...
sudo systemctl stop monitor-ssh-telegram.service

sudo tee /usr/local/bin/monitor-ssh-telegram.sh >/dev/null <<'EOF'
#!/usr/bin/env bash
...
sudo chmod 755 /usr/local/bin/monitor-ssh-telegram.sh
sudo systemctl daemon-reload
sudo systemctl restart monitor-ssh-telegram.service
sudo journalctl -u monitor-ssh-telegram.service -n 50 --no-pagerSCRIPT_PATH="$(readlink -f "${BASH_SOURCE[0]:-$0}")"
if [ ! -f "$SCRIPT_PATH" ]; then
  SCRIPT_PATH=""
fi
SCRIPT_URL="https://raw.githubusercontent.com/farshadasadpour/freshvps/main/monitor-ssh-telegram.sh"
